...
uint64_t test_pattern = COLOR_WHITE;
uint32_t test_pattern_lsb = COLOR_WHITE;
uint32_t test_pattern_msb = COLOR_WHITE;
...
//#define LED_PORTS       8
uint16_t led_patterns[LED_PORTS][LED_NUM][LED_CHANNELS] = {0};
//int LED_WIDTH = 40;
//int LED_HEIGHT = 24;
...
static inline void put_pixel(uint32_t pixel_grb) {
    ...
    pio_sm_put_blocking(pio0, 0, 0x2ff);
    pio_sm_put_blocking(pio0, 1, 0x2ff);
    pio_sm_put_blocking(pio0, 2, 0x2ff);
    pio_sm_put_blocking(pio0, 3, 0x2ff );
    pio_sm_put_blocking(pio1, 0, 0x2ff);
    pio_sm_put_blocking(pio1, 1, 0x2ff);
    pio_sm_put_blocking(pio1, 2, 0x2ff);
    pio_sm_put_blocking(pio1, 3, 0x2ff);
#endif
}

static inline void put_pixel_by_panel_64bits(uint8_t panel_id, uint64_t msb, uint32_t lsb) {
    /*printf("pixel_grb = 0x%x\n", pixel_grb); 
    uint32_t lsb =  pixel_grb & 0x00000000ffffffff;
    uint32_t msb = (pixel_grb & 0xffffffff00000000) >> 32;
    printf("lsb = 0x%x\n", lsb); 
    printf("msb = 0x%x\n", msb); */
    switch(panel_id){
        case 0:
            //pio_sm_put_blocking(pio0, 0, pixel_grb << 16u);
            //pio_sm_put_blocking(pio0, 0, 0x4 << 8u);
            //pio_sm_put_blocking(pio0, 0, 0xff << 8u);
            pio_sm_put_blocking(pio0, 0, msb << 8u);
            pio_sm_put_blocking(pio0, 0, lsb << 8u);
            break;
        case 1:
            //pio_sm_put_blocking(pio0, 1, pixel_grb << 16u);
            pio_sm_put_blocking(pio0, 1, lsb << 8u);
            pio_sm_put_blocking(pio0, 1, msb << 8u);
            break;
        case 2:
            //pio_sm_put_blocking(pio0, 2, pixel_grb << 16u);
            pio_sm_put_blocking(pio0, 2, msb << 8u);
            pio_sm_put_blocking(pio0, 2, lsb << 8u);
            break;
        case 3:
            //pio_sm_put_blocking(pio0, 3, pixel_grb << 16u);
            pio_sm_put_blocking(pio0, 3, msb << 8u);
            pio_sm_put_blocking(pio0, 3, lsb << 8u);
            break;
        case 4:
            //pio_sm_put_blocking(pio1, 0, pixel_grb << 16u);
            pio_sm_put_blocking(pio1, 0, msb << 8u);
            pio_sm_put_blocking(pio1, 0, lsb << 8u);
            break;
        case 5:
            //pio_sm_put_blocking(pio1, 1, pixel_grb << 16u);
            pio_sm_put_blocking(pio1, 1, msb << 8u);
            pio_sm_put_blocking(pio1, 1, lsb << 8u);
            break;
        case 6:
            //pio_sm_put_blocking(pio1, 2, pixel_grb << 16u);
            pio_sm_put_blocking(pio1, 2, msb << 8u);
            pio_sm_put_blocking(pio1, 2, lsb << 8u);
            break;
        case 7:
            //pio_sm_put_blocking(pio1, 3, pixel_grb << 16u);
...
static inline void put_pixel_by_panel(uint8_t panel_id, uint64_t pixel_grb) {
    switch(panel_id){
        case 0:
            //pio_sm_put_blocking(pio0, 0, pixel_grb << 16u);
            //pio_sm_put_blocking(pio0, 0, 0x4 << 8u);
...
int main(void) {
    int i, j, k;
    uint32_t r_br_level = 0;
    uint32_t g_br_level = 0;
    uint32_t b_br_level = 0;
    uint32_t color_interval = 0;
    stdio_init_all();
    ...
    while (1) {
        ...
        //sem_release(&led_frame_sem);
        color_interval ++;
        if(color_interval >= 800){
            color_interval = 0;
        }
        if((0 <= color_interval) && (color_interval < 200)){
            r_br_level = 32;
            g_br_level = 0;
            b_br_level = 0;
        }else if((200 <= color_interval) && (color_interval < 400)){
            r_br_level = 0;
            g_br_level = 32;
            b_br_level = 0;
        
        }else if((400 <= color_interval) && (color_interval < 600)){
            r_br_level = 0;
            g_br_level = 0;
            b_br_level = 32;
        }else if((600 <= color_interval) && (color_interval < 800)){
            r_br_level = 32;
            g_br_level = 32;
            b_br_level = 32;
        } 
        ...
        if(led_lighting_mode == LED_NORMAL_MODE){
	        for(j = 0; j < LED_HEIGHT; j++){
	            for(i = 0; i < LED_WIDTH; i++){
                    for(k = 0; k < LED_PORTS; k++){
                        /* RED*/
                        /*test_pattern_msb = (br_level & 0x0000ffff) << 8;       
                        test_pattern_lsb = 0;*/
                        /*GREEN*/
                        /*test_pattern_msb = (br_level >> 8) & 0x0000ffff;       
                        test_pattern_lsb = (br_level & 0x000000ff) << 16;*/
                        /*BLUE*/
                        /*test_pattern_msb = 0;       //b
                        test_pattern_lsb = br_level;*/
                        /*WHITE*/
                        /*test_pattern_msb = ((br_level & 0x0000ffff) << 8) | ((br_level >> 8) & 0x0000ffff);       
                        test_pattern_lsb = ((br_level & 0x000000ff) << 16) | br_level;*/
#if 1
                        
                        /*r_br_level = led_patterns[k][((j*led_total_width) + i)][0] ;       //b
                        g_br_level = led_patterns[k][((j*led_total_width) + i)][1] ;       //b
                        b_br_level = led_patterns[k][((j*led_total_width) + i)][2] ;       //b*/
                        test_pattern_msb = ((r_br_level & 0x0000ffff) << 8) | ((g_br_level >> 8) & 0x0000ffff);       
                        test_pattern_lsb = ((g_br_level & 0x000000ff) << 16) | b_br_level;
#else
                        if(j < 6){
                            /*Green*/
                            test_pattern_msb = (br_level >> 8) & 0x0000ffff;       
...
#endif
	    	            put_pixel_by_panel_64bits(k, test_pattern_msb, test_pattern_lsb);
                    }
	            }
	        }
            sleep_ms(10);
                        ...
                        if((j >= led_area_starty)&&(j < (led_area_starty + led_area_height))){
                            if(j % 2 == 0){
                                if((i >= led_area_startx)&&(i < (led_area_startx + led_area_width))){
                                    test_pattern = led_patterns[k][((j*led_total_width) + i)][0] << 32 |  //g
                                                led_patterns[k][((j*led_total_width) + i)][1] << 16 |  //r
